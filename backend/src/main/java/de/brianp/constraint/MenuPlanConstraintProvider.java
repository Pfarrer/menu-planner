...
public class MenuPlanConstraintProvider implements ConstraintProvider {
    ...
    private Constraint duplicateRecipeOnSameDay(ConstraintFactory constraintFactory) {
        return constraintFactory.forEach(MenuItem.class).groupBy(MenuItem::getDay, MenuItem::getRecipe)
                .filter((day, recipe) -> recipe != null)
                .penalize("Duplicate recipe on same day", HardSoftScore.ONE_HARD, (day, recipe) -> 1);
    }

    private Constraint minimizePrepTime(ConstraintFactory constraintFactory) {
        return constraintFactory.forEach(MenuItem.class).penalize("Minimize prep time", HardSoftScore.ONE_SOFT,
